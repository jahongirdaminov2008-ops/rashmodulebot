...
async def is_subscribed(user_id: int) -> bool:
    try:
        ...
        logging.warning(f"Obuna tekshirishda xatolik (user_id={user_id}, CHANNEL_ID={CHANNEL_ID}): {e}")
        return False
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Admin obuna tekshiruvidan chetlab o'tadi
    if is_admin(user_id):
        await message.answer("Salom, admin!", reply_markup=main_menu(user_id))
        return
